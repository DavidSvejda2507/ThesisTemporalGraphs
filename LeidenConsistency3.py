...
class LeidenClass:
    ...
    def updateOptions(self, none_option, previous_options, local_options, interactions, graph_id):
        """Combines the possible moves so far with the possible moves in the next graph to find new possible moves.

        Args:
            none_option (namedtuple): Option that corresponds to not moving the previous vertex.
            previous_options (list[namedtuple]): List of all of the best moves for move in the previous graph.
        ...
        option = namedtuple("option", ["target", "graph", "target_comms", "dQ"])
        next_options = []
                
        for local_opt in local_options:
            max_dQ = none_option.dQ + sum(local_options[local_opt])
            best_option = none_option
            for opt in previous_options:
                dQ = opt.dQ + sum(local_options[local_opt]) + interactions[opt.target][local_opt]
                if dQ > max_dQ:
                    max_dQ = dQ
                    best_option = opt
            targets = best_option.target_comms.copy()
            targets.append(local_opt)
            new_opt = option(local_opt, graph_id, targets, max_dQ)
            next_options.append(new_opt)
            
        max_dQ = none_option.dQ
        best_option = none_option
        for opt in previous_options:
            if opt.dQ > max_dQ:
                max_dQ = opt.dQ
        ...
        targets = best_option.target_comms.copy()
        targets.append(None)
        new_none_opt = option(None, graph_id, targets, max_dQ)
        
        return new_none_opt, next_options
    ...
    def localMoveVertex(self, graph_id, vertex_id, queue):
        ...
        for direction, limit in [(-1, -1), (1, len(self.graph_stack[0]))]:
            ...
            for current_graph_id in range(graph_id+direction, limit, direction):
                current_vertex_id = self.matchToNextGraph(previous_graph_id, current_graph_id, previous_vertex_id)
                selected_vertices[current_graph_id] = current_vertex_id
                
                previous_vertex_id = current_vertex_id
                previous_graph_id = current_graph_id
            
        option = namedtuple("option", ["target", "graph", "target_comms", "dQ"])
        none_opt, options = option(None, -1, [], 0), []
        ...
        for graph_id in range(len(graphs)):
            ...
            local_options, next_comms = self.findMoves(graph, vertex_id, graph_id, community_edges_dict)
            ...
            # Update options to new graph
            none_opt, options = self.updateOptions(none_opt, options, local_options, interactions, graph_id)
            
            previous_comms = next_comms
            
        for index, graph in enumerate(graphs):
            graph.vs[selected_vertices[index]][self._queued] = False
                
        final_option = max([none_opt] + options, key = lambda x:x.dQ)
